    ...
    public class OrderController : ApiController
    {
            ...
            try
            {
                ...
                objOrder = (from p in _db.tbl_Orders
                            ...
                            select new OrderVM
                            {
                                ...
                                OrderShipCity = p.OrderShipCity,
                                OrderShipState = p.OrderShipState +"- INDIA",
                                OrderShipAddress = p.OrderShipAddress,
                                OrderPincode = p.OrderShipPincode,
                                OrderShipClientName = p.OrderShipClientName,
                                OrderShipClientPhone = p.OrderShipClientPhone,
        ...
        public ResponseDataModel<string> SaveShippingCharge(GeneralVM objGen)
        {
            ...
            try
            {
                ...
                if (WalletUsed > 0 && Onlinpymt == 0)
                {
                    ...
                    {
                        if (objclientuss.WalletAmt != null && objclientuss.WalletAmt >= WalletUsed)
                        {
                            IsValidWallet = true;
                        }
                        else
                    ...
                    {
                        response.AddError("Wallet balance not sufficient");
                    }
                ...
                if (WalletUsed > 0 && Onlinpymt == 0)
                {
                    ...
                    if (IsValidWallet == true)
                    {
                        ...
                        if (WalletUsed > 0)
                        {
                            ...
                            objPyment1.PaymentBy = "wallet";
                            objPyment1.AmountDue = Convert.ToDecimal(amountdue);
                            objPyment1.AmountPaid = Convert.ToDecimal(WalletUsed);
                            objPyment1.DateOfPayment = DateTime.UtcNow;
                            objPyment1.CreatedBy = UserId;
                            objPyment1.CreatedDate = DateTime.UtcNow;
                            ...
                            objPyment1.RazorpayPaymentId = "";
                            objPyment1.RazorSignature = "";
                            objPyment1.PaymentFor = "Order Amount";
                            _db.tbl_PaymentHistory.Add(objPyment1);
                            ...
                            objwlt.ClientUserId = UserId;
                            objwlt.WalletDate = DateTime.UtcNow;
                            objwlt.Description = "Paid Order Amount for order no." + objOrder.OrderId;
                            _db.tbl_Wallet.Add(objwlt);
                            objclientuss.WalletAmt = objclientuss.WalletAmt - WalletUsed;
                            _db.SaveChanges();
                            objCom.SavePaymentTransaction(0, objOrder.OrderId, true, WalletUsed, "Payment By Wallet", UserId, false, DateTime.UtcNow, "Wallet");
                            objCom.SaveTransaction(0, 0, objOrder.OrderId, "Order Amount Payment By Wallet : Rs" + WalletUsed, WalletUsed, UserId, 0, DateTime.UtcNow, "Wallet Payment");
                        }
                        response.Data = "Success";
                    }
                    else
                    ...
                    if (objpymn != null)
                    {
                        if (objpymn["status"] != null && Convert.ToString(objpymn["status"]) == "captured")
                        {
                            ...
                            if (WalletUsed > 0)
                            {
                                ...
                                objCom.SavePaymentTransaction(0, objOrder.OrderId, true, WalletUsed, "Payment By Wallet", UserId, false, DateTime.UtcNow, "Wallet");
                                objCom.SaveTransaction(0, 0, objOrder.OrderId, "Order Amount Payment By Wallet : Rs" + WalletUsed, WalletUsed, UserId, 0, DateTime.UtcNow, "Wallet Payment");
                                amountdue = amountdue - WalletUsed;
                            }
                            ...
                            _db.tbl_PaymentHistory.Add(objPayment);
                            _db.SaveChanges();
                            objCom.SaveTransaction(0, 0, orderid64, "Due Order Amount Paid Online: Rs" + Onlinpymt, Onlinpymt, UserId, 0, DateTime.UtcNow, "Amount Due Paid");
                            objCom.SavePaymentTransaction(0, orderid64, true, Onlinpymt, "Payment By Online for Due Amount", UserId, false, DateTime.UtcNow, "Online Payment");
                            response.Data = "Success";
                        }
                    }
                }
        ...
        public ResponseDataModel<string> CreateRazorPayOrder(GeneralVM objGen)
        {
            ...
            try
            {
                ...
                decimal wltamt = 0;
                wltamt = Convert.ToDecimal(objGen.AmountPayByWallet);
        ...
        public ResponseDataModel<string> AssignDelieveryPerson(GeneralVM objGen)
        {
            ...
            try
            {
                ...
                using (WebClient webClient = new WebClient())
                {
                    //string msg = "Order no." + OrdId + " \nItem: " + ItmsText + " \nhas been assigned to you for delivery";
                    int SmsId = (int)SMSType.AgentAssignDelieveryToStaff;
                    clsCommon objcm = new clsCommon();
                    string msg = objcm.GetSmsContent(SmsId);
        ...
        public ResponseDataModel<string> SetDeliveredItems(GeneralVM objGen)
        {
            ...
            try
            {
                ...
                using (WebClient webClient = new WebClient())
                {
                    ...
                    int SmsId = (int)SMSType.WhenDelivered;
                    clsCommon objcm = new clsCommon();
                    string msg = objcm.GetSmsContent(SmsId);
                    msg = msg.Replace("{{OrdeNo}}", OrdrId + "").Replace("{{ItemText}}", ItmsText);
                    msg = HttpUtility.UrlEncode(msg);
        ...
        public ResponseDataModel<OtpVM> SendOTPForDelivery(OtpVM objOtpVM)
        {
            ...
            try
            {
                ...
                using (WebClient webClient = new WebClient())
                {
                    ...
                    {
                        int SmsId = (int)SMSType.OtpDuringDelivery;
                        clsCommon objcm = new clsCommon();
        ...
        [Route("DownloadInvoiceForItem"), HttpPost]
        public ResponseDataModel<string> DownloadInvoiceForItem(GeneralVM objGen)
        {
            ResponseDataModel<string> response = new ResponseDataModel<string>();
            try
            {
                long OrderId = Convert.ToInt64(objGen.OrderId);
                long OrderItemId = Convert.ToInt64(objGen.OrderDetailId);
                StreamReader sr;
                string newhtmldata = "";
                ...
                if (objOrder != null)
                {
                    ...
                    {
                        file = HttpContext.Current.Server.MapPath("~/templates/InvoiceCancel.html");
                    }
                    else if (objItem.ItemStatus == 6 && objItem.IsDeleted == true)
                    {
                        file = HttpContext.Current.Server.MapPath("~/templates/InvoiceReturn.html");
                    }
                    else if (objItem.ItemStatus == 8 && objItem.IsDeleted == true)
                    {
                        file = HttpContext.Current.Server.MapPath("~/templates/InvoiceExchange.html");
                    }
                    string GSTTitle = "GST";
                    if (objOrder.OrderShipState != "Gujarat")
                    {
                ...
                // set PDF page margins
                htmlToPdfConverter.Document.Margins = new PdfMargins(5);
                string invfile = "Inv" + objOrder.OrderId +"_"+ OrderItemId+".pdf";
                // convert HTML code to a PDF memory buffer
                htmlToPdfConverter.ConvertHtmlToFile(newhtmldata, "", HttpContext.Current.Server.MapPath("~/Documents/") + invfile);
        ...
        [Route("DownloadPaymentReport"), HttpPost]
        public ResponseDataModel<string> DownloadPaymentReport(GeneralVM objGen)
        {
            ResponseDataModel<string> response = new ResponseDataModel<string>();
            try
            {
                long clientuserid = Convert.ToInt64(objGen.ClientUserId);
                string PaymentMode = objGen.paymentmode;
                ...
                string flname = "PaymentReport_" + clientuserid + "_" + Guid.NewGuid().ToString()+".xlsx";
        ...
        [Route("GetWalletAmt"), HttpPost]
        public ResponseDataModel<GeneralVM> GetWalletAmt(GeneralVM objGen)
        {
            ResponseDataModel<GeneralVM> response = new ResponseDataModel<GeneralVM>();
            try
            {
                long UserId = Convert.ToInt64(objGen.ClientUserId);
                var objclientuss = _db.tbl_ClientUsers.Where(o => o.ClientUserId == UserId).FirstOrDefault();
                decimal CurrWltAmt = 0;
                if(objclientuss != null)
                {
                    if(objclientuss.WalletAmt != null)
                    {
                        CurrWltAmt = objclientuss.WalletAmt.Value;
                    }                    
                }
                GeneralVM objG = new GeneralVM();
                objG.TotalWalletAmt = Convert.ToString(CurrWltAmt);
                response.Data = objG;
